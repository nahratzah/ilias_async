	...
	\transformation{abuse:transition}
		...
		\assume{abuse:assume-A1}
			...
			\step{}
			    ...
			    {$\land$-intro on \ref{abuse:assume-A1} and \ref{abuse:transition}}
			\skipsteps{2}{\vdots}{}
		...
		\assume{abuse:assume-not-A2}
			...
			\step{abuse:assume-not-A2-false}
			    {\neg \mathcal{A}_2 \land \mathcal{T} \implies \false}
...
\end{flagderiv}
Any predicate crossing into the transition boundary has to be re-evaluated
using proof by contradiction.
Thus $\mathcal{T}$ acts as a filter on each predicate prior to the transition.
...
limited to the input set $\mathbb{R}$.
...
\end{equation*}
Which translates to:\\
define a transition: $s \becomes \sqrt{r}$\\
...
after the transition $a \becomes \sqrt{b}$.
...
\introduce{abuseExample:intro}
	...
	\transformation{abuseExample:transition}
	    {a \becomes \sqrt{b} \yields a = \sqrt{b}}
	    {
		Introduce transformation on $\mathcal{S}$
		(substituting $a$ for $s$ and $b$ for $r$)
	    }
		\assume{abuseExample:pull}
		    {
			\neg \left( a = 1 \land b = 2 \right)
		    }
		    {
			Test if $b = 2$ still holds after the transition
			from \ref{abuseExample:intro}
		    }
			\step{abuseExample:pullBpost}
			    {
				\neg \left( a = 1 \land b = 2 \right)
				\land
				a = \sqrt{b}
		...
		\assume{abuseExample:pullA}
		    {a = 1}
		    {Test if $a = 1$ still holds after the transition
		      from \ref{abuseExample:intro}}
			\step{abuseExample:tPost}
			    {a = \sqrt{b}}
			    {Use postcondition from
			      \ref{abuseExample:transition}}
			\step{abuseExample:introAndOnPullAAndTPost}
			    {a = \sqrt{b} \land a = 1}
			    {$\land$-intro on \ref{abuseExample:pullA}
			      and \ref{abuseExample:tPost}}
			\step{abuseExample:substB}
			    {}
			    {Substitute $b = 2$ on
			      \ref{abuseExample:introAndOnPullAAndTPost}}
\conclude[2]{}
    {
	\hoaretrip
	    {
    ...
    {Hoare triple using \ref{abuseExample:intro}, \ref{abuseExample:transition}, ...}
\end{flagderiv}
